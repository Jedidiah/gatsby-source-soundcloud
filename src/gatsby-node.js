...
const createHash = (obj) => crypto.createHash('md5')
...
const addIfUnique = (uniqueItems, item) => (uniqueItems.find(t => t.id === item.id))
    ? uniqueItems
    : [...uniqueItems, item];
  ...
  return nodes.map((n) => {
    ...
    if (n.tracks) {
      ...
      n.tracks.map((t) => {
        ...
        if (track) { addReverseLink(n, track); }
        if (user) { addReverseLink(n, user); }
      });
